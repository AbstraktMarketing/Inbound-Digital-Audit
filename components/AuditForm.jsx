...
export default function AuditForm({ onSubmit, theme: t }) {
  ...
  const [form, setForm] = React.useState({
    ...
    semrushProjectId: "",
    linkedinUrl: "", facebookUrl: "", blogUrl: "",
  });
  ...
  function renderStep() {
    ...
    if (step === 3) return (
      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        <div>
          <label style={{ ...labelStyle, display: "flex", alignItems: "center", gap: 6 }}>
            SEMrush Project ID {optionalBadge}
          </label>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
          <div>
            <label style={{ ...labelStyle, display: "flex", alignItems: "center", gap: 6 }}>
              LinkedIn URL {optionalBadge}
            </label>
            <input type="text" value={form.linkedinUrl} placeholder="linkedin.com/company/acme"
              onChange={e => update("linkedinUrl", e.target.value)} style={inputStyle("linkedinUrl")}
            />
          </div>
          <div>
            <label style={{ ...labelStyle, display: "flex", alignItems: "center", gap: 6 }}>
              Facebook URL {optionalBadge}
            </label>
            <input type="text" value={form.facebookUrl} placeholder="facebook.com/acmecorp"
              onChange={e => update("facebookUrl", e.target.value)} style={inputStyle("facebookUrl")}
            />
          </div>
        </div>
        <div>
          <label style={{ ...labelStyle, display: "flex", alignItems: "center", gap: 6 }}>
            Blog URL {optionalBadge}
          </label>
          <input type="text" value={form.blogUrl} placeholder="https://example.com/insights"
            onChange={e => update("blogUrl", e.target.value)} style={inputStyle("blogUrl")}
          />
          <div style={{ fontSize: 10, color: t.subtle, marginTop: 5, lineHeight: 1.4 }}>
            If your blog lives at a custom path (not /blog), enter the full URL so we detect your content.
          </div>
        </div>
      </div>
    );
